...
# ─── SUPER-ADMINS ─────────────────────────────────────────────────────────────
# These IDs have god-mode access across all groups — broadcast, addadmin, etc.
# Hardcoded founders; additional super-admins are stored in the DB.
FOUNDER_IDS: set[int] = {6066940244, 7902074220}

KRAVEN_CHANNEL = os.getenv("KRAVEN_CHANNEL", "https://t.me/kraven")  # set in .env
...
def init_db():
    with db() as conn:
        ...
        -- Super-admins added dynamically (founders are hardcoded)
        ...
        -- Broadcast log for reach tracking
    ...
    with db() as conn:
        ...
        # Migration: add points_per_link to topic_settings if upgrading from older version
        cols = [r[1] for r in conn.execute("PRAGMA table_info(topic_settings)").fetchall()]
        if "points_per_link" not in cols:
            conn.execute("ALTER TABLE topic_settings ADD COLUMN points_per_link INTEGER DEFAULT 10")
            conn.commit()
            logger.info("Migrated topic_settings: added points_per_link column.")
...
def is_super_admin(conn, user_id: int) -> bool:
    """Returns True if user is a founder or a dynamically added super-admin."""
...
def track_group(conn, chat_id: int, chat_title: str):
    """Record this group so broadcasts can reach it."""
    conn.execute(
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        ...
        # Track this group for broadcasts
...
async def cmd_addadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /addadmin [user_id]
    Grant super-admin access to a Telegram user ID.
    Only founders and existing super-admins can run this.
    """
    tg_user = update.effective_user
    with db() as conn:
        ...
        conn.execute(
            "INSERT OR IGNORE INTO super_admins (user_id, added_by) VALUES (?,?)",
...
async def cmd_removeadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /removeadmin [user_id]
    Revoke super-admin access. Cannot remove founders.
    """
    tg_user = update.effective_user
    with db() as conn:
        if not is_super_admin(conn, tg_user.id):
        ...
        conn.execute("DELETE FROM super_admins WHERE user_id=?", (target_id,))
        conn.commit()
...
async def cmd_listadmins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = ["*Super-Admins*\n"]
    lines.append("Founders (hardcoded):")
    ...
    if rows:
        lines.append("\nDynamically added:")
        for row in rows:
            lines.append(f"  {row['user_id']} — added by {row['added_by']} on {row['added_at'][:10]}")
...
async def cmd_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /broadcast [message]
    Send a sponsored message to every known group.
    Super-admins only.
    """
    tg_user = update.effective_user
    with db() as conn:
        ...
        if not raw:
            await update.message.reply_text(
                ...
                "Example: /broadcast Alpha Drop campaign is live — join now!"
            )
            return
    ...
    if not groups:
        await update.message.reply_text("No groups on record yet. Bot needs to be active in groups first.")
        return
    ...
    with db() as conn:
        conn.execute(
            "INSERT INTO broadcast_log (sent_by, message, groups_count) VALUES (?,?,?)",
            (tg_user.id, raw, sent),
        )
        conn.commit()
...
async def cmd_broadcaststats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for row in logs:
        ...
        lines.append(f"\n{row['sent_at'][:16]} — {row['groups_count']} groups\n_{preview}_")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Log all unhandled exceptions so they surface in Railway logs with context."""
